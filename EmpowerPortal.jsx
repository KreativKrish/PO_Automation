...
const MOCK_PRFS = [
  {
    ...
    remarks: [
      ...
      { by: "System", at: "2026-02-01T14:30:00Z", text: "Notified Finance Reviewer. Status → Pending Review." },
    ],
    history: [
      { status: "New", at: "2026-01-28T09:14:00Z" },
      { status: "Vendor Validation", at: "2026-01-28T09:15:00Z" },
      { status: "Pending Review", at: "2026-02-01T14:30:00Z" },
    ],
    docs: [
      { name: "Approved PRF", type: "prf", url: "#" },
      { name: "Vendor PAN Card", type: "vendor", url: "#" },
    ],
  },
  {
    id: "PRF-2526-002",
    ...
    history: [
      ...
      { status: "Vendor Validation", at: "2026-01-30T11:23:00Z" },
    ],
    docs: [{ name: "Approved PRF", type: "prf", url: "#" }],
  },
  {
    id: "PRF-2526-003",
    ...
    history: [
      ...
      { status: "Vendor Validation", at: "2026-01-25T08:01:00Z" },
      { status: "Pending Review", at: "2026-01-28T09:00:00Z" },
      { status: "Approved", at: "2026-02-02T10:10:00Z" },
      { status: "PO Generated", at: "2026-02-02T10:15:00Z" },
    ],
    docs: [
      { name: "Approved PRF", type: "prf", url: "#" },
      { name: "Vendor PAN Card", type: "vendor", url: "#" },
      { name: "MSME Declaration", type: "vendor", url: "#" },
      { name: "Generated PO", type: "po", url: "#" },
    ],
  },
  {
    id: "PRF-2526-004",
    ...
    history: [
      ...
      { status: "Vendor Validation", at: "2026-01-22T16:46:00Z" },
      { status: "Pending Review", at: "2026-01-25T09:00:00Z" },
      { status: "Rejected – Action Required", at: "2026-01-29T11:00:00Z" },
    ],
    docs: [{ name: "Approved PRF", type: "prf", url: "#" }],
  },
  {
    ...
    glCode: "GL-5028",
    nature: "Service",
    poNumber: "TLE-FIN-2526-005",
    poDate: "2026-01-31",
    createdAt: "2026-01-18T10:00:00Z",
    updatedAt: "2026-01-31T15:20:00Z",
    remarks: [
      { by: "System", at: "2026-01-18T10:00:00Z", text: "PRF auto-detected." },
      { by: "Vivek", at: "2026-01-31T15:15:00Z", text: "Approved. PO generated and synced to SAGE." },
    ],
    history: [
      { status: "New", at: "2026-01-18T10:00:00Z" },
      { status: "Vendor Validation", at: "2026-01-18T10:01:00Z" },
      { status: "Pending Review", at: "2026-01-22T09:00:00Z" },
      { status: "Approved", at: "2026-01-31T15:15:00Z" },
      { status: "PO Generated", at: "2026-01-31T15:20:00Z" },
    ],
    docs: [
      { name: "Approved PRF", type: "prf", url: "#" },
      { name: "Vendor PAN Card", type: "vendor", url: "#" },
...
const MOCK_VENDORS = [
  { code: "VND-0412", name: "CloudHost Solutions", pan: "ABCDE1234F", gstin: "18ABCDE1234F1Z5", status: "active", msme: true },
  { code: "VND-0198", name: "MediaWorks Studio", pan: "LMNOP9012Q", gstin: "18LMNOP9012Q1Z5", status: "active", msme: false },
  { code: "VND-0301", name: "FinSoft Ltd", pan: "RSTUV3456W", gstin: "18RSTUV3456W1Z5", status: "active", msme: true },
  { code: "VND-0505", name: "PixelCraft Agency", pan: "XYZAB7890C", gstin: "18XYZAB7890C1Z5", status: "active", msme: false },
  { code: null, name: "DevTools Inc", pan: "FGHIJ5678K", gstin: null, status: "pending_sage", msme: false },
];

// ─── STATUS CONFIG ────────────────────────────────────────────────────────────
const STATUS_MAP = {
  new: { label: "New", color: "#60a5fa", bg: "#dbeafe" },
  vendor_validation: { label: "Vendor Validation", color: "#a78bfa", bg: "#ede9fe" },
  pending_review: { label: "Pending Review", color: "#fb923c", bg: "#ffedd5" },
  rejected: { label: "Rejected – Action Required", color: "#f87171", bg: "#fee2e2" },
  approved: { label: "Approved", color: "#34d399", bg: "#d1fae5" },
  po_generated: { label: "PO Generated", color: "#14b8a6", bg: "#ccfbf1" },
  closed: { label: "Closed", color: "#9ca3af", bg: "#f3f4f6" },
  new_pending: { label: "New – VRF Pending", color: "#a78bfa", bg: "#ede9fe" },
};

const STATUS_LIFECYCLE = ["New", "Vendor Validation", "Pending Review", "Approved", "PO Generated", "Closed"];
...
function TopBar({ title }) {
  ...
  return (
    <header style={{ position: "fixed", top: 0, left: T.sidebarW, right: 0, height: T.headerH, background: "#fff", borderBottom: `1px solid ${T.cardBorder}`, display: "flex", alignItems: "center", gap: 16, padding: "0 28px", zIndex: 90, boxShadow: "0 1px 4px rgba(0,0,0,.06)" }}>
      <h1 style={{ fontSize: 18, fontWeight: 700, color: T.textMain, margin: 0, flex: 0 }}>{title}</h1>
      {/* Search */}
...
function PRFDetail({ prf, onBack, onUpdatePrf }) {
  ...
  const [showModal, setShowModal] = useState(null); // 'approve' | 'reject' | 'edit'
  const [editData, setEditData] = useState({ qty: prf.qty, rate: prf.rate, glCode: prf.glCode, nature: prf.nature });
  ...
  const handleAction = (action) => {
    ...
    const newRemarks = [...prf.remarks, { by: "Vivek", at: now, text: remarks }];
    ...
    if (action === "approve") {
      ...
      newHistory.push({ status: "Pending Review (Edited)", at: now });
    }
    onUpdatePrf({ ...prf, ...editData, poValueExGst, gstAmount, totalPoValue, status: newStatus, remarks: newRemarks, history: newHistory });
    setRemarks("");
    setShowModal(null);
    setEditing(false);
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <h2 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: T.textMain }}>{prf.id}</h2>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 18 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          {/* PRF Info (read-only) */}
          <Card>
            <CardHeader icon="file_text" title="PRF Information" />
            <div style={{ padding: "0 20px 18px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px 24px" }}>
              {[
                ["Department", prf.dept],
                ["Cost Code", prf.costCode],
                ["Project / University", prf.project],
                ["Purpose", prf.purpose],
                ["Service From", fmt(prf.servicePeriod.from)],
                ["Service To", fmt(prf.servicePeriod.to)],
              ].map(([label, val]) => (
                <div key={label}>
          ...
          <Card>
            <CardHeader icon="orders" title="Financial Details" action={
              !editing && prf.status === "pending_review" && (
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }}>
          <div style={{ background: "#fff", borderRadius: 12, width: 420, padding: 28, boxShadow: "0 20px 60px rgba(0,0,0,.2)" }}>
            <h3 style={{ margin: "0 0 6px", fontSize: 17, color: T.textMain }}>
              {showModal === "approve" ? "Approve PRF" : showModal === "reject" ? "Reject PRF" : "Save Edits"}
            </h3>
            <p style={{ margin: "0 0 14px", fontSize: 13, color: T.textSub }}>
              {showModal === "approve" ? "This will approve the PRF and trigger PO generation." : showModal === "reject" ? "The requester will be notified via email." : "Edited details will be saved and status remains Pending Review."}
            </p>
            {!remarks.trim() && <div style={{ background: "#fef3c7", border: "1px solid #f59e0b", borderRadius: 7, padding: "8px 12px", fontSize: 12.5, color: "#92400e", marginBottom: 12, display: "flex", alignItems: "center", gap: 6 }}><Icon name="alert" size={14} color="#92400e" /> Remarks are mandatory.</div>}
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 18 }}>
              ...
              <button onClick={() => handleAction(showModal)} disabled={!remarks.trim()} style={{ background: showModal === "reject" ? T.danger : T.primary, color: "#fff", border: "none", borderRadius: 7, padding: "8px 20px", fontSize: 13, fontWeight: 600, cursor: remarks.trim() ? "pointer" : "not-allowed", opacity: remarks.trim() ? 1 : 0.5 }}>
                Confirm {showModal === "approve" ? "Approval" : showModal === "reject" ? "Rejection" : "Edit"}
              </button>
            </div>
          </div>
        </div>
...
function VendorsPage() {
  ...
  const filtered = filter === "all" ? MOCK_VENDORS : MOCK_VENDORS.filter((v) => v.status === filter);
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18 }}>
        {["all", "active", "pending_sage"].map((f) => (
          <button key={f} onClick={() => setFilter(f)} style={{ background: filter === f ? T.primary : "#fff", color: filter === f ? "#fff" : T.textSub, border: `1px solid ${filter === f ? T.primary : T.cardBorder}`, borderRadius: 7, padding: "6px 14px", fontSize: 13, fontWeight: filter === f ? 600 : 500, cursor: "pointer" }}>
            {f === "all" ? "All" : f === "active" ? "Active" : "Pending SAGE"}
          </button>
        ))}
      </div>
      <Card>
        <CardHeader icon="vendors" title="Vendor Master" />
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ background: T.bg }}>
                {["Vendor Code", "Vendor Name", "PAN", "GSTIN", "MSME", "Status"].map((h) => (
            ...
            <tbody>
              {filtered.map((v, i) => (
                <tr key={v.name} style={{ background: i % 2 === 0 ? "#fff" : "#fafbfc", borderBottom: `1px solid ${T.cardBorder}` }}>
                  ...
                  <td style={{ padding: "11px 16px", color: T.textSub }}>{v.pan}</td>
                  <td style={{ padding: "11px 16px", color: T.textSub }}>{v.gstin || "—"}</td>
                  <td style={{ padding: "11px 16px" }}><span style={{ fontSize: 11.5, fontWeight: 600, color: v.msme ? "#16a34a" : T.textSub }}>{v.msme ? "Yes" : "No"}</span></td>
      ...
      case "prfs": return (
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <span style={{ fontSize: 14, color: T.textSub }}>{prfs.length} Purchase Requisition Forms</span>
          </div>
          <Dashboard prfs={prfs} onSelectPrf={setSelectedPrf} />
        </div>
      );
